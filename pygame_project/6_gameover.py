...
# 화면 타이틀 설정
pygame.display.set_caption("Dong Pang") # 게임 이름
...
image_path = os.path.join(current_path, "images") # imges 폴더 위치 반환
...
# 2. 이벤트 처리 (키보드, 마우스 등)
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                ...
                weapon_x_pos = character_x_pos + (character_with /2) - (weapon_wiht / 2)
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        #가로벽에 닿았을 때 공 이동 위치 변경 (팅겨 나오는 효과)
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                    ...
                    if ball_img_idx < 3:
                        ...
                        small_ball_heidth = small_ball_rect.size[1]

                        # 왼쪽으로 튕겨나가는 작은 공
                        balls.append({
                            "pos_x" : ball_pos_x + (ball_width / 2) - (small_ball_width / 2), # 공의 x 좌표
                            "pos_y" : ball_pos_y + (ball_height / 2) - (small_ball_heidth / 2), # 공의 y 좌표
        ...
        break # 안쪽 for 문에서 break 를 만나면 여기로 진입 가능. 2중 for 문을 한번에 탈출 가능
